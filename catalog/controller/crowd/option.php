<?php
class ControllerCrowdOption extends Controller {
	private $error = array();
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			$this->load->language('crowd/option');
			...
			$options = $this->model_crowd_option->getOptions($filter_data);

			foreach ($options as $option) {
				$option_value_data = array();

				if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
					$option_values = $this->model_crowd_option->getOptionValues($option['option_id']);

<?php
class ControllerCrowdProduct extends Controller {
	private $error = array();
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$addproduct['mpn'] = '';
			$addproduct['location'] = '';
			$addproduct['product_store'] = array(0);
...
*/
	protected function getForm() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('crowd/product')
		);

		if (!isset($this->request->get['product_id'])) {
			$data['action'] = $this->url->link('crowd/product/add');
		} else {
			$data['action'] = $this->url->link('crowd/product/edit');
		}
		...
		*/
		...
		if (isset($this->request->post['product_store'])) {
			...
			$data['product_store'] = $this->model_crowd_product->getProductStores($this->request->get['product_id']);
		} else {
			$data['product_store'] = array(0);
		}
		...
		$this->load->model('crowd/recurring');

		$data['recurrings'] = $this->model_crowd_recurring->getRecurrings();

		if (isset($this->request->post['product_recurrings'])) {
			$data['product_recurrings'] = $this->request->post['product_recurrings'];
		} elseif (!empty($product_info)) {
			$data['product_recurrings'] = $this->model_crowd_product->getRecurrings($product_info['product_id']);
		} else {
			$data['product_recurrings'] = array();
		}
/*
		$this->load->model('crowd/tax_class');

		$data['tax_classes'] = $this->model_crowd_tax_class->getTaxClasses();
...
/*
		...
		$this->load->model('crowd/stock_status');

		$data['stock_statuses'] = $this->model_crowd_stock_status->getStockStatuses();
		...
		$this->load->model('crowd/weight_class');

		$data['weight_classes'] = $this->model_crowd_weight_class->getWeightClasses();
		...
		$this->load->model('crowd/length_class');

		$data['length_classes'] = $this->model_crowd_length_class->getLengthClasses();
...
*/
		// Categories
		$this->load->model('crowd/category');

		if (isset($this->request->post['product_category'])) {
			$categories = $this->request->post['product_category'];
		} elseif (isset($this->request->get['product_id'])) {
			$categories = $this->model_crowd_product->getProductCategories($this->request->get['product_id']);
		} else {
			$categories = array();
		}
		...
		foreach ($categories as $category_id) {
			$category_info = $this->model_crowd_category->getCategory($category_id);
...
*/
		// Options
		$this->load->model('crowd/option');

		if (isset($this->request->post['product_option'])) {
			$product_options = $this->request->post['product_option'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_options = $this->model_crowd_product->getProductOptions($this->request->get['product_id']);
		} else {
		...
		foreach ($data['product_options'] as $product_option) {
			if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
				if (!isset($data['option_values'][$product_option['option_id']])) {
					$data['option_values'][$product_option['option_id']] = $this->model_crowd_option->getOptionValues($product_option['option_id']);
				}
			}
		}
/*
		...
		*/
		...
		if (isset($this->request->post['product_image'])) {
			...
			$product_images = $this->model_crowd_product->getProductImages($this->request->get['product_id']);
		} else {
...
*/
		...
		$this->response->setOutput($this->load->view('crowd/product_form', $data));
	}
